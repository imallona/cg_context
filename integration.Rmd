...
for (item in samples_annot$sample) {
    ...
    if (basename(fn) %in% basename(fns)) {
        ...
        toy <- fread(sprintf('zcat %s |sed "s/ /\t/g"', fn),
                     header = FALSE,
                     sep = '\t',
        ...
        fd[[fn]]$factor_w <- substr(tolower(as.character(fd[[fn]]$seq_w)), 2, 7)
        fd[[fn]]$factor_c <- substr(tolower(as.character(fd[[fn]]$seq_c)), 2, 7)
