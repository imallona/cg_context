...
```{r xyplots, include = TRUE, echo = TRUE, cache = TRUE, cache.lazy = FALSE,
...
```{r pheatmap, include = TRUE, echo = TRUE, cache = TRUE, cache.lazy = FALSE,
fig.width = 12, fig.height = 12}         
...
plot(m1.margins)
cplot(m1.margins, 'motif') 
cplot(m1.margins, 'genotype') 
    ...
    file.rename(
        ...
        file.path(
            sprintf('logo_%s_%s.pdf', ssample,
                    as.character(samples_annot[samples_annot$sample == ssample, 'genotype'])), WD))
}
