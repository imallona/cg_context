...
Per Baubec request (October 17th 2019). Largely based upon `cph_quickchanges_simpler.Rmd`; older report available at [http://imlstaupo.uzh.ch/imallona/baubec/cg_context/cph_quickchanges.html](taupo).

Requires `extract_motifs_frequency_from_bam_binary.sh` to be run first.

Per request, should contain the data already here plus the following
...
```{r, include = FALSE}
annot <- read.table(text = "20181207.A-WGBS_IL12,WT_DNMT3A2_in_TKO,bwa_hiseq4k_pe
20181207.A-WGBS_IL14,QC_DNMT3A2_in_TKO,bwa_hiseq4k_pe
20181207.A-WGBS_IL18,WT_DNMT3A2_in_TKO,bwa_hiseq4k_pe
20181207.A-WGBS_IL19,QC_DNMT3A2_in_TKO,bwa_hiseq4k_pe
20180913.A-WGBS_3,DNMT3A2-mutant_in_TKO,bwa_hiseq4k_pe
20180913.A-WGBS_4,DNMT3A2-mutant_in_TKO,bwa_hiseq4k_pe
...
20190524.A-TBWGBS_A,neuro_DNMT3A_addback,bwa_hiseq2500_se
20190524.A-TBWGBS_B,neuro_DNMT3B_addback,bwa_hiseq2500_se
20190524.A-TBWGBS_K,neuro_DNMT3A_KO,bwa_hiseq2500_se
20190524.A-TBWGBS_W,neuro_WT,bwa_hiseq2500_se",
col.names = c('sample', 'id', 'tech'),
sep = ',')
```

Note thta the `merged` datasets correspond to:
...
We get the nucleotide weights for all the methylated and unmethylated motifs as when building a PWM. Getting the log2 enrichment of the meth/unmeth ratio.
...
Let's visualize the calculations for sample `tko+d3a1_merged` and a context `CG`

```{r}

context <- 'cg' ; genotype <- 'tko+d3a1_merged'
...
for (context in names(d)) {
    ...
    print(ggplot(melted2, aes(position, deviation, fill=as.factor(sample)))+
          geom_bar(position="dodge",stat="identity") +
          scale_x_continuous(breaks = 1:8) +
          facet_wrap(~nucleotide,nrow = 4) + xlab("position") +
          ylab('log2fc meth/unmeth') +
          ggtitle(context) +
          guides(fill = guide_legend(nrow = 12)) + 
          theme(legend.position="bottom", legend.title=element_text(size=5), 
                legend.text=element_text(size=5), legend.key.size = unit(1,"line")))
