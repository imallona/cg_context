...
Within reads (with at least 10 events)
...
within <- within[within$cometh.n >= 10,]
```
...
```{r}
fit <- lm(x$cometh ~ x$read + x$type)

## https://rdrr.io/cran/rsq/man/rsq.partial.html
summary(fit)
print(rsq::rsq.partial(fit))
```


```{r}
fit2 <- lm(x$cometh ~ x$read * x$type)

summary(fit2)
print(rsq::rsq.partial(fit2))
```



```{r}
fit3 <- lm(x$cometh ~ x$read + x$motifs)


summary(fit3)

print(rsq::rsq.partial(fit3))
```
...
top <- head(bg[order(bg$meth.mean, decreasing = TRUE), 'motif'], 1)
...
```{r}
fit <- lm(x$cometh ~ as.factor(x$has_top))
...
Within reads (with at least 10 events in the genome)
