---
title: "CG and CH seq preferences with DNMT binding masks (no coverage filtering)"
...
Requires `motif_extract_binding_marks_august_2020` to be run first. This contains (without coverage filtering) motif calls stratified by genomic regions (DNMT bound).
...
`R_LIBS=/home/Shared_taupo/Rlib/release-3.11-lib /usr/local/R/R-4.0.0/bin/R`
...
PARENT <- '/home/Shared_s3it/imallona'
TASK <- "cg_context"
SUBTASK <- 'nar_review'

## WD <-  file.path(PARENT, 'mnt', 'nfs', TASK)
WD <-  file.path(PARENT, TASK, SUBTASK)

## DATA <- file.path(PARENT, 'data')

## MIN_DEPTH <- 10
## NFS <- file.path(PARENT, 'mnt', 'nfs')
...
```{r, include = FALSE}
annot <- read.table(text = "20151223.B-MmES_TKOD3A1c1-3_R,tko+d3a1,bwa_hiseq2k,+a1,es
...
Add masks (naming/generation by Tuncay)

```{r}
masks <- c('NEW_DNMT3A1_sites', 'NEW_DNMT3A2_sites', 'NEW_DNMT3B_sites')

annot <- merge(annot, expand.grid(annot$sample, masks), by.x = 'sample', by.y = 'Var1')

colnames(annot) <- c('sample', 'id', 'tech', 'id_short', 'celltype', 'mask')
row.names(annot) <- paste(annot$sample, annot$mask, sep = '/')
annot$compound_id <- rownames(annot)
...
knitr::include_graphics("/home/imallona/src/cg_context/media/stranded_dnameth.png")
...
sources <- list.files(WD, recursive = TRUE, pattern = 'motif_counts_*')
...
[Download ](counts_nested_list_with_masks.RData)

```{r saving_r_data}

save(x = d, file = file.path(WD, 'counts_nested_list_with_masks.RData'))
## str(d)
...
Let's visualize the calculations for sample `tko+d3a2_merged` in `NEW_DNMT3B_sites` and a context `CG`

```{r}

context <- 'cg' ; genotype <- 'tko+d3a2_merged/NEW_DNMT3B_sites'
...
```{r, fig.height =8, fig.width = 5, results = 'asis'}
for (context in names(outer)) {
    ...
    melted2 <- merge(melted2, annot, by.x = 'sample', by.y = 'compound_id', all.x = TRUE)
   
    print(ggplot(melted2, aes(position, deviation, fill=as.factor(nucleotide)))+
          geom_bar(position="dodge",stat="identity") +
          scale_x_continuous(breaks = 1:8) +
          facet_grid(sample.y+mask~nucleotide) +
          xlab("position") +          
...
for (context in names(counts)) {
    ...
    sums <- merge(sums, annot, by.x ='sample', by.y='compound_id')
    print(ggplot(sums,
...
for (context in names(counts)) {
    ...
    detects <- merge(detects, annot, by.x ='sample', by.y='compound_id')
...
Sorted by log2FC in nucleotide C2

```{r, fig.height = 4, fig.width = 7, results = 'asis'}
for (context in names(d)) {
    cat('### ', context, '\n\n')
    ...
    melted2$tiny <- dirname(as.character(melted2$sample))
        
    print(ggplot(melted2, aes(position, deviation, fill=as.factor(nucleotide)))+
          geom_bar(position="dodge",stat="identity") +
          scale_x_continuous(breaks = 1:8) +
          facet_grid(mask~tiny) +
          xlab("position") + ylab('log2fc meth/unmeth') +
          ggtitle(context))
...
for (context in names(d)) {
    ...
    ratios <- sapply(d[[context]], function(x) return(x$meth$count))/
         (sapply(d[[context]], function(x) return(x$unmeth$count)) + sapply(d[[context]], function(x) return(x$meth$count)) ) 

     rownames(ratios) <- d[[context]][[1]]$unmeth$motif
...
## Pheatmaps without scaling

```{r, fig.height = 8, fig.width = 8, cache = FALSE, results = 'asis'}
...
for (context in names(d)) {
    ...
    annot2 <- annot
    rownames(annot2) <- annot$compound_id
    annot2 <- annot2[,c('sample', 'mask')]
    annot2$sample <- dirname(annot2$sample)


    pheatmap(na.omit(ratios), cluster.rows = TRUE, cluster.cols = TRUE, annotation_col = annot2,
...
# Heatmaps by nucleotide and sample representations


```{r, results='asis', fig.width = 3, fig.height = 2, cache = FALSE, results = 'asis'}
for (context in names(d)) {
