...
for (context in names(nd)) {
    ...
    ggplot(data = melted, aes(x = motif, y = value, group = variable, color = variable))+ 
        geom_point()
    ...
    ggplot(data = melted, aes(reorder(fac, value, mean), y = value, group = variable,
                              color = variable))+ 
        geom_point()  
...
for (context in names(nd)) {
    ...
    ## now pca
    autoplot(prcomp(selected), label = TRUE,
             main = sprintf('q99 variable motifs, n = %s', nrow(selected)))
    autoplot(prcomp(t(selected)), label = TRUE, loadings = TRUE,
             loadings.label = TRUE, loadings.label.size = 3,
             main =  sprintf('q99 variable motifs, n = %s', nrow(selected)))

    ## pca on the whole stuff
    rownames(nd[[context]]) <- nd[[context]]$motif
    autoplot(prcomp(nd[[context]][,-1]), label = TRUE,
             main = 'all motifs')
    
    autoplot(prcomp(t(nd[[context]][,-1])), label = TRUE, loadings = TRUE,
             loadings.label = TRUE, loadings.label.size = 3,
             main =  'all motifs')
...
for (context in names(topmotifs)) {
    for (cutoff in names(topmotifs[[context]])) {
        ...
        for (genotype in levels(melted$variable)) {
            ...
            ## lbits[[genotype]] <- ggseqlogo(melted[melted$variable == genotype, 'motif'], method = 'bits')
            ## lprobs[[genotype]] <- ggseqlogo(melted[melted$variable == genotype, 'motif'],
            ##                                 method = 'probability')
        ...
        print(cutoff)
        ggseqlogo(lprobs, ncol = 1, method = 'probability')
        ggseqlogo(lprobs, ncol = 1, method = 'bits')
